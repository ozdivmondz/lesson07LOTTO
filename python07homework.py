...
p1 = 15
p2 = 15
bochonocks = random.sample(range(90), 90)
...
def you_move():
    ...
    if a == 'y':
        if bochonock in p1_set:
            for l in p1_field:
                try:
                    l.insert(l.index(bochonock), '><')
                    l.pop(l.index(bochonock))
                except ValueError:
                    continue
            print('\nПродолжаем...')
            return 1
...
def com_move():
    ...
    if bochonock in p2_set:
        for i in p2_field:
            try:
                i.insert(i.index(bochonock), '><')
                i.pop(i.index(bochonock))
            except ValueError:
                continue
        return 1
...
for bochonock in bochonocks:
    bochonocks_left -= 1
    print('\nНовый бочонок: {} (осталось: {})\n'.format(bochonock, bochonocks_left))
    field(0)
    field(1)
    if you_move() == 1:
        p1 -= 1
